...
//Calculate Brooks-Corey ks (cm/hr), bubbling pressure (cm), pore-size distribution index, and residual water content
	...
	std::cout << pSand << '\t' << pClay << '\t' << ks << '\t' << bubbling_pressure << '\t' << pore_size_index << '\t' << residual << endl;
	std::cout << endl;
	if (treesParams.useHydraulics == true)
	{
		std::cout << ">>> TREES, v. 3.1.5, deterministic mode with plant water balance <<< \n\n";
	}
	else
	{
		std::cout << ">>> TREES, v. 3.1.5, deterministic mode without plant water balance <<< \n\n";
	}
	std::cout << endl;
//Calculate field capacity
	fieldCapacity = field_capacity(bubbling_pressure, pore_size_index, residual, treesParams.porosity);
	...
	{
		std::cout << "Running with Bryophytes\n";
	}
	//std::cout << "ks = " << ks << endl;
	//std::cout << "bubbling_pressure = " << bubbling_pressure << endl;
	//std::cout << "pore_size_index = " << pore_size_index << endl;
	//std::cout << "residual = " << residual << endl;
	std::cout << "fc = " << fieldCapacity << endl;
...
std::cout << "rootDiamMultiplier = " << treesParams.rootDiamMultiplier << endl;
std::cout << "rootArea = " << rootArea << endl;
...
//call simulations functions for current time step
			...
			fluxout << simOut.A_sun << '\t' << simOut.A_shd << '\t' << simOut.Av << '\t' << simOut.Aj << '\t' << simOut.phi2 << '\t' << simOut.betaA << '\t' << simOut.L_sun << '\t' << simOut.L_shd << '\t' << simOut.T_sun << '\t' << simOut.T_shd << '\t' << simOut.D0_sun << '\t' << simOut.D0_shd << '\t' << simOut.Ci_sun << '\t' << simOut.Ci_shd << '\t' << simOut.PAR_sun << '\t' << simOut.PAR_shd << '\t' << simOut.gs_sun << '\t' << simOut.gs_shd << '\t' << simOut.NEE << '\t';
