...
//get input
	std::cout << "Observation File Name: ";
	std::cin >> dfile;
...
//Plant Shoot and Root Module Parameter Read In
	...
	if(mp > npostp)
	{
		...
		std::cout << mp << " required " << npostp << " available\n";
		return 0;
	}
	std::cout << endl << pdfile << " OK\n";
	std::cout << "How many simulations for each parameter set in posterior file? ";
	std::cin >> nperp;
...
//prepare for simulations
	...
	{
		std::cout << "ERROR opening output file for ET " << outfile << endl;
		return 0;
	}
	if(open_outfile(outfile2, out2)==0)
	{
		std::cout << "ERROR opening output file for NEE " << outfile2 << endl;
		return 0;
	}
