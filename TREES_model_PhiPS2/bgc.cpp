...
{
	std::cout << "Clearing out BGC" << endl;
...
BiogeochemicalCycles::~BiogeochemicalCycles()
{
	std::cout << "Deconstructing bgc" << endl;
	clearPools();
}
...
std::cout << "projected canopy ground area = " << projGNDarea << endl;;
...
std::cout << "leafBiomassCarbon = " << leafBiomassCarbon[0] << endl;
...
std::cout << "leafBiomassNitrogen = " << leafBiomassNitrogen[0] << endl;
...
//stem [N] is assumed to be low (0.3% of carbon)
		...
		chloroplastSugar[0] = 0.0;
		CN = leafBiomassCarbon[0]/leafBiomassNitrogen[0];
std::cout << "leafBiomassCarbon[0] = " << leafBiomassCarbon[0] << "; " << leafBiomassNitrogen[0] << "; CN = " << CN << endl;
...
//std::cout << rhizosphereLiveMicrobialCarbon[j][k] << '\t';
...
//std::cout << glycineNitrogen[0] << '\t' << glycineCarbon[0] << '\t' << serineNitrogen[0] << '\t' << serineCarbon[0] << endl;
}
...
//DK(3/14/2022): update Rd
	//Rd25 = treesParams.Rd_mult * Vcmax25; //leaf mitochondrial respiration, umol m-2 s-1
	//E_Rd = treesParams.E_Rd;       //activation energy for leaf mitochondrial respiration, kJ mol-1
	//Rd = Rd25 * exp((tk-298.0)*E_Rd*1000.0/(298.0*R*tk));
...
//therefore, Ile no longer used, but embedded in phiJ
	...
	// DK(7/22/2021): non-linear fit for PhiPS2 ~ LWP under a reference light
	phi2_hyd = 0.05 + (0.63 - 0.05) * exp(-pow(-leafpsi/0.98,3.44));
	betaA = log((phi2_hyd - 0.048)/(0.79 - 0.048))/800.0;
	phi2 = (0.79 - 0.048) * exp(betaA*in.irad) + 0.048;
...
//Cuticular conductance
	//DK(2/17/2022): removed Gsref0
	if (g < 0.005/1.6/42.0)
	  //if (g < 0.01*treesParams.Gsref0/1.6/42.0)
        {
	  g = 0.005/1.6/42.0;
	  // g = 0.01*treesParams.Gsref0/1.6/42.0;
                out.g = g;
        }
        D = in.D;
        E = 1.6*g*D;
...
std::cout << "canopy_cover = " << canopy_cover << endl;
        }
        else
        {
                canopy_cover = 1.0;
...
void BiogeochemicalCycles::updateLeafCarbonNitrogenPools(int k,
							 ...
							 double& N_pos_demand)
{
	double CN, nsc, delta_nsc, deltaBiomassC, deltaBiomassN, GRcost, SLA_instant;
...
//
	...
	if (stemCincrement > 0.99*1.17*CN*leafStoredNitrogen[0])
	{
		stemCincrement = 0.99*1.17*CN*leafStoredNitrogen[0];
	}
	...
	liveStemCarbon[0] += stemCincrement/1.17;
	liveStemNitrogen[0] += stemCincrement/CN/1.17;
	leafStoredNitrogen[0] -= stemCincrement/CN/1.17;
...
//
			...
			{
				std::cout << "BiogeochemicalCycles::updateRootCarbonNitrogenPools():" << endl;
				std::cout << leafCfraction << '\t' << stemAllocation << '\t' << rgrowth << '\t' << tgrowth << '\t' << N_avail_rate_plant << '\t' << rootAllocation << '\t' << CN << endl;
				exit(1);
			}
...
//std::cout << "Cb = " << Cb << "; Ch = " << Ch << "; Cl = " << Cl << endl;
					...
					trees_params treesParams)
{
    	...
    	//std::cout << "SLA at this ts of lf is " << SLA_instant << endl;
...
// if the change will bring total greater than max
            		...
            		//std::cout << "cost of cd, kgC, = " << Rcd << endl;
        	}
...
//added - DSM
  //          		SingleLeafBiomassNitrogen[k] +=  deltaN; // structural N, in kgC
            		//std::cout << "deltaN at this ts is " << deltaN << endl;
