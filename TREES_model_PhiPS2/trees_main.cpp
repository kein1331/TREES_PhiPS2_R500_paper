...
//using std::* is to replace the issue with switching to cmath
...
//For hydraulic model
	...
	std::cout << "Number of time steps to aggregate for MCMC evaluations (suggest 1, 2, 4, 6, 8, 12):";
	std::cin >> n_agg;
...
*/
        std::cout << dfile << " contains " << indata.no_of_vars() << " variables " << 
							nsteps << " data points" << endl;
	std::cout << n_agg << " steps will be aggregated to produce " << nsteps/n_agg << 
...
//Plant Shoot and Root Module Parameter Read In
        std::cout << "Shoot and Root Modules File Name: ";
        std::cin >> param_file_modules;
...
//Parameter File Validation - **no validation done for shoot and root module read in (add later)
	...
	if(rp>0)//MCMC
	{
		...
		strcat(cfile, ".chn");
		strcat(efile, ".eff");
		std::cout << "# of iterations to run: ";
		std::cin >> total_sims;
		std::cout << total_sims << endl;
		std::cout << "Initial covariance matrix file: ";
		std::cin >> vfile;
		std::cout << vfile << endl;
		std::cout << "Minimum target efficiency: ";
		std::cin >> mineff;
		std::cout << mineff << endl;
		std::cout << "Maximum target efficiency: ";
		std::cin >> maxeff;
		std::cout << maxeff << endl;
		std::cout << "# of samples to use for covariance updating: ";
		std::cin >> nsamples;
		std::cout << nsamples << endl;
		std::cout << "covariance update interval: ";
		std::cin >> updtg;
		std::cout << updtg << endl;
		std::cout << "use alternating chains for ET and NEE (1=yes; 0=no): ";
		std::cin >> use_alternating_chains;
		std::cout << use_alternating_chains << endl;
//end user input
		...
		effout << "output file: " << cfile << endl;
        	...
        	std::cout << endl;
        	std::cout << ">>> TREES, v. 3.1.5, stochastic parameter mode <<< \n\n";
        	std::cout << endl;

		std::cout << "Running MCMC - " << total_sims << " iterations\n";
...
//switch likelihood function each time covariance is updated
	...
	else if (rp == 0)//simulation with constant parameters
	{
		...
		fluxout << "J_sun\tJ_shd\tAsun\tAshd\tAv\tAj\tphi2\tbetaA\tLsun\tLshd\tTsun\tTshd\tDsun\tDshd\tCi_sun\t";
