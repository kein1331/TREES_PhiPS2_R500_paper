...
//for large changes in soil water potential, also adjust precision of E increment
		...
		if (!silent)
		{
			if (xylemScalar < 1.0)
			{
				std::cout << endl << "-> Predawn: updating Weibulls with xylem loss = " << 
			...
			{
				//std::cout << endl << "-> Predawn: updating Weibulls with soil Psi change = " <<
				//		 delta << endl;
			}
		}
...
//dF[k,m]/dY[k,m+1]
    			...
    			innerWhile1: std::cout <<"";
	

                    	if ( error == true || nits>MAXITS )
			{
                      		//std::cout << "exiting innerWhile2\t"<<nits<<"\n";
                        	goto innerWhile2;
                    	}
...
//reset water contents of all ELEMENTS
    		...
    		innerWhile2: std::cout <<"";
            	if ( (nits > MAXITS) || (h > MAXH) || error) 
		{
			//std::cout << "Exit outer\t"<<ssss<<"\t"<<h<<" \n";
                	goto outerWhile;
            	}
...
//This is when the final pressures for the rhizosphere nodes are outputted.  DER 6/25/08
            	...
            	for ( i = 1; i <= ShootModules; i++) 
		{
	    	...
	    	//std::cout << "pleafave " << pleafave << endl;
	    	//std::cout << "ptarg " << ptarg << endl;
            	//if (pleafave == ptarg) {
