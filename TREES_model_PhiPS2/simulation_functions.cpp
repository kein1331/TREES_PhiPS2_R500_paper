...
//**********************************************************************************************
	...
	double daily_md_lwp;  // for beta PS model linked to plant hydraulic status
	double daily_pd_lwp;  // for beta PS model
...
//display current status of the larget live LAI and stress on the the canopy
        ...
        if ((hour+min) == 5.0 && treesParams.useHydraulics == true && !silent)
	{
		...
		std::cout << year << ":" << yday;
		std::cout << " --------" << endl;
		std::cout << "target LAI = " << treesParams.live_lai;
		std::cout << "; canopy PLC = " << 100.0*(1.0-k_p_e.latK[1]/ksat[1][1]);
		std::cout << "; N status (mol N+ mol organic N) = " << bgc.plantNstatus[0] << endl;
	}

	// Used to calc midday leaf water potential to pass into beta PS model for reducing beta decay rate on PSII efficiency JRP 10/18
	if (treesParams.usePhenology == false && treesParams.useLeafModule == true)
	  {

	    //added to handle leaf water potential
	    ...
	    if ((hour+min) == 13.0 && treesParams.useHydraulics == true)
	      {
		simOut.daily_md_lwp = simOut.Leaf_Psi;
	      }
...
// checkpoint: enter repro phase?
            	if (treesParams.therm_plant >= treesParams.floweringTime)
             	{
                	std::cout << "Enter Reproductive Phase" << endl;
            	} 
...
// co-opted code to get thetaRoot for updateLeaf function
							...
							bgc.Lf_idx[0]-1, thetaRootTmp);
                        		...
                        		//std::cout << "leaf " << k << " has potA of " << bgc.getSingleLeafAreaPotential(k) << endl;
                    		}
                	}
		}
...
// plant-level SLA
            	if (bgc.Lf_idx[0] > 0 && bgc.getSingleLeafArea(0)> 0)
            	{
                	...
                	//std::cout << "plant level SLA at this ts is " << bgc.calcPlantSLA(bgc.Lf_idx[0], treesParams) << "\n";
                	//std::cout << "plant level SLA at this ts is " << treesParams.SLA << "\n";
            	}

// plant-level leaf biomass C
            	//std::cout << "leaf biomass c = " << bgc.getLeafBiomassCarbon() << endl;

// plant-level leaf biomass N
            	//std::cout << "leaf biomass N = " << bgc.getLeafBiomassN() << endl;
...
//std::cout << "nodeFail = " << nodeFail[1] << endl;
...
//Plant water balance failure, use previous state, try a lower stomatal conductance
				std::cout << "*** Hydraulic failure - attempting to recover ***\n";
                    	}
		}
...
//std::cout << leafBiomassC << '\t' << leafBiomassN << '\t' << leafBiomassN/max(0.000000001,leafArea) << '\t' << Nleaf*Nrubisco*fnr*treesParams.act25*1.0e6 / 60.0<< endl;
		in.lnc = Nleaf;
...
//calculate sun leaf photosynthesis, intercellular CO2, and stomatal conductance
		...
		if (treesParams.ps_model == 1 || treesParams.ps_model == 2) //C3 photosynthesis
		{
			...
			{
			  A_sun = bgc.photosynthesis(treesParams, Jmax_mult, thetaJ, adj_phiJ_sun, leafpsi, in, out);
...
//calculate shade leaf photosynthesis, intercellular CO2, and stomatal conductance
		...
		if (treesParams.ps_model == 1 || treesParams.ps_model == 2) //C3 photosynthesis
		{
			if (treesParams.ps_model == 1) //use the original TREES photosynthesis model (pre-2019)
			{
			  A_shd = bgc.photosynthesis(treesParams, Jmax_mult, thetaJ, adj_phiJ_shd, leafpsi, in, out);
...
//std::cout << ModelStatus << "; A_tot = " << A_tot << "; Gs = " << simOut.Gs << endl;
...
//we'll write out the updated Weibull parameters, b and c at 1pm each day
	...
	{
		std::cout << "Plant hydraulic status (Ks and Weibulls):" << endl;
		std::cout << "\tShoot:\t" << "axial Ks=" << ks2[1][0] << ", b=" << b[1][0] << 
				", c=" << cc[1][0] << "; lateral Ks=" << ks2[1][1] << 
				", b=" << b[1][1] << ", c=" << cc[1][1] << endl;
		for (i = 0; i < rmodules; i++)
		{
			std::cout << "\tRoot(" << i << ")\t" << "axial Ks=" << ks2[i+3][0] << 
...
//std::cout << litter_store/litter_capacity << endl;
//std::cout << litter_store << '\t' << litter_capacity << '\t' << Rnet_soil << '\t' << t_surface << '\t' << Dsurface << '\t' << gva << '\t' << lai << '\t' << canopy_cover << endl;
...
/*
std::cout << bgc.getChloroplastStarch() << '\t';
	bgc.putChloroplastStarch(bgc.getChloroplastStarch()-rgrowth/nscRatio);
std::cout << bgc.getChloroplastStarch() << endl;
*/
...
//Low NSC is less than 2% during cold periods or less 0.5% anytime
		if (treesParams.forceRefilling == true && treesParams.useHydraulics == true)
		{
				...
				nsc/(Croot+Cstem+treesParams.lai/treesParams.SLA*10000.0)*100.0 < 2.0)
			{
				...
				if (!silent)
				{
					std::cout << "*** Re-filling turned off ***" << endl;
				}
			}
...
//If NSC recovers to at least 0.5% of live tissue then restore refilling
								...
								t_root >= 10.0 && kratio >= 0.5)
			{
				...
				if (!silent)
				{
					std::cout << "*** Re-filling turned on ***" << endl;
				}
			}
			if (kratio < 0.2)
			{
...
//std::cout << A_tot << '\t' << simOut.NPP << '\t' << 0.14*rgrowth * 4.6296 << endl;
